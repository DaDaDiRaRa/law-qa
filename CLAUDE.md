...
python -m scripts.fetch_laws --commit                            # 핵심 국가법령 — 아직 미실행

python -m scripts.fetch_ordinances --dry-run
python -m scripts.fetch_ordinances --sido 서울특별시 --commit   # 특정 시도만
python -m scripts.fetch_ordinances --commit                     # 광역시·도 전체 — 아직 미실행

python -m scripts.fetch_ordinances --sigungu 서울특별시 --dry-run
python -m scripts.fetch_ordinances --sigungu 서울특별시 --commit  # 시군구 단위 (서울 완료)

python -m scripts.fetch_supplementary --dry-run
python -m scripts.fetch_supplementary --commit                  # 부수법령 — 아직 미실행
```

### 현재 DB 적재 현황 (2026-06-01 기준)
...
│   │   ├── fetch_laws.py           ← 국가법령 수집 (법제처 API) — 미실행
│   │   ├── fetch_ordinances.py     ← 자치법규 수집 (--sigungu 플래그로 시군구 지원)
│   │   ├── fetch_supplementary.py  ← 부수법령 수집 (법제처 API) — 미실행
