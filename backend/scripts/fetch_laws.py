#!/usr/bin/env python3
"""법제처 DRF API로 법령 조문 수집 후 law_qa.db에 적재.

사용법:
  python -m scripts.fetch_laws --dry-run   # 파싱 결과만 출력
  python -m scripts.fetch_laws --commit    # 실제 DB 저장
"""
...
_REQ_INTERVAL = 0.5   # API 요청 간격 (초) — 과부하 방지
...
def _find_mst(law_name: str) -> str | None:
    """법령명으로 MST(법령 고유번호) 조회. 정확히 일치하는 항목만 반환."""
...
def _build_article_no(조번호: str, 조문제목: str) -> str:
    base = f"제{조번호}조"
    return f"{base}({조문제목})" if 조문제목 else base
...
def _collect_unit(unit: ET.Element, prefix: str, title: str, source: str) -> dict | None:
    ...
    article_no = prefix + _build_article_no(조번호, 조문제목)

    # 조문내용이 이미 "제N조(…)" 형태 헤더를 포함하면 그대로 사용
...
def _parse_law(xml_bytes: bytes, law_name: str, source: str) -> list[dict]:
    ...
    seen: dict[str, dict] = {}   # article_no → row (중복 시 마지막 우선)
...
def main() -> None:
    ...
    print(f"수집 대상: {len(TARGETS)}개 법령\n")
